...
class Node {
	constructor(name) {
		...
		this.netint = Array.from({length: Math.floor(Math.random() * Math.min(Math.max(localStorage.maxLink ??  4, 2), 4))}, (_) => new Interface(this.id))
		this.name = name
		this.enabled = false
	}
}
...
function randNetwork() {
	...
	d.nodes.forEach((s) => {d.nodes.forEach((t) => {mkl(d, s, t)})})
	console.log(d)
...
function draw(force) {
	...
	n.append("circle")
	 .attr("r", 32)
     .attr("fill", (d) => {return d.enabled ? "green":"red"})
	n.append("text")
